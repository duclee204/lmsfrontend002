...
export class CourseManagementComponent implements OnInit, AfterViewInit {
  ...
  loadCategories(): void {
    ...
    this.http.get<any[]>('https://lms-backend001-110ad185d2b7.herokuapp.com/api/categories/list', { headers })
      .subscribe({
        next: (data) => {
...
updateCourse(): void {
  ...
  this.http.put(`https://lms-backend001-110ad185d2b7.herokuapp.com/api/courses/${this.selectedCourseId}`, formData, {
    responseType: 'text'
  }).subscribe({
    next: (res) => {
